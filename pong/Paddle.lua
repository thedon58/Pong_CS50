...
function Paddle:ai()
    if gameState == 'play' then
        predictedx = self.x + (self.width/2);
        slope = ball.dy / ball.dx
        predictedy = slope * (predictedx-ball.x) + ball.y
        frac = 5
        topsr = self.y + (self.height / frac)
        bottsr = self.y + ((frac - 1)*(self.height/frac))
        if predictedy > bottsr then
            self.dy = PADDLE_SPEED / 5
        elseif predictedy < topsr then
            self.dy = -PADDLE_SPEED / 5
        else
            self.dy = 0
        end
